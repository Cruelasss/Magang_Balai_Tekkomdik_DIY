...
const Mentors = () => {
  ...
  const [newMentor, setNewMentor] = useState({ nama_pembimbing: '', divisi: '', max_kuota: 5 });

  // --- STATE UNTUK MONITORING PESERTA ---
  const [selectedMentor, setSelectedMentor] = useState(null);
  const [isDetailOpen, setIsDetailOpen] = useState(false);
  const [participants, setParticipants] = useState([]);
  
  // State Modal Detail Peserta (Control Center)
  ...
  // 1. Fungsi Buka Daftar Mahasiswa Bimbingan
 // 1. Fungsi Buka Daftar Mahasiswa Bimbingan
  const handleShowParticipants = async (mentor) => {
    ...
    try {
      ...
      // PERBAIKAN: Hanya filter status 'Aktif'
      // Kita gunakan toLowerCase() supaya aman jika di database ada variasi penulisan (Aktif/aktif)
  ...
  // 2. Fungsi Buka Control Center Peserta
  const openInternControl = (intern) => {
    setSelectedIntern(intern);
    setActiveTab('profil');
    setIsControlOpen(true);
    setLogs([]); // Reset logs saat ganti peserta
  };
    ...
    <div className="p-8">
      ...
      <div className="flex justify-between items-center mb-10">
        <div className="text-left">
          ...
          <p className="text-sm text-gray-500 font-medium">Monitoring pembimbing dan laporan mahasiswa aktif.</p>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={() => setIsModalOpen(true)} className="bg-blue-600 text-white px-5 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-blue-700 transition-all shadow-lg shadow-blue-100">
          ...
          <div key={m.id} className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between">
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex gap-2">
                ...
                <button className="p-2 bg-gray-50 text-gray-400 rounded-lg hover:bg-red-50 hover:text-red-500 transition-all"><Trash2 size={18} /></button>
              </div>
            </div>
      ...
      {/* MODAL 1: DAFTAR MAHASISWA (HEADER GELAP) */}
      {isDetailOpen && selectedMentor && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-[#1e293b] rounded-[32px] w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in duration-200">
            <div className="p-6 text-white flex justify-between items-center">
              <div className="text-left">
                <h2 className="text-xl font-bold uppercase tracking-tight">{selectedMentor.nama_pembimbing}</h2>
                <p className="text-[10px] text-gray-400 uppercase font-black tracking-[0.2em] mt-1">Daftar Bimbingan Aktif</p>
              </div>
              <button onClick={() => setIsDetailOpen(false)} className="hover:bg-white/10 p-2 rounded-full transition-colors"><X size={24}/></button>
            </div>
            
            <div className="p-6 bg-white rounded-t-[32px] min-h-[350px]">
                  ...
                  {participants.map((p, index) => (
                    <div key={p.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-blue-200 transition-all">
                      <div className="flex items-center gap-4 text-left">
                        <div className="w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center font-black text-sm">{index + 1}</div>
                        <div>
                          <h4 className="font-bold text-gray-800 text-sm uppercase leading-tight">{p.nama}</h4>
                          <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mt-0.5">{p.instansi} • {p.jurusan}</p>
                        </div>
                      </div>
                      <button onClick={() => openInternControl(p)} className="flex items-center gap-2 px-4 py-2 bg-white text-blue-600 border border-blue-100 rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-blue-600 hover:text-white transition-all shadow-sm">
                        <BookOpen size={14} /> Logbook
                      </button>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="py-24 text-center text-gray-400 italic text-sm font-medium">Belum ada mahasiswa bimbingan.</div>
              )}
              <div className="mt-8">
                 <button onClick={() => setIsDetailOpen(false)} className="w-full bg-[#1e293b] text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] hover:bg-black transition-all">Tutup Menu</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* MODAL 2: CONTROL CENTER (TANPA KELOLA MENTOR) */}
                ...
                <div className="space-y-4 animate-in slide-in-from-bottom-2 duration-300">
                  ...
                  {logs.length > 0 ? (
                      ...
                      {logs.map(log => (
                        <div key={log.id} className="p-6 border border-gray-100 rounded-[28px] bg-gray-50/50 hover:bg-white hover:shadow-md transition-all">
                          <div className="flex justify-between items-center mb-4">
                            ...
                            <span className={`text-[9px] font-black px-4 py-1.5 rounded-full uppercase tracking-tighter ${log.status_validasi === 'Valid' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{log.status_validasi || 'MENUNGGU'}</span>
                          </div>
                    ...
                    <div className="py-24 text-center">
                      ...
                      <p className="text-gray-400 italic font-medium">Belum ada aktivitas yang dilaporkan hari ini.</p>
                    </div>
                  )}
                </div>
              )}
