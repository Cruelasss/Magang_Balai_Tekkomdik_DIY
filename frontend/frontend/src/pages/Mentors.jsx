...
const Mentors = () => {
  ...
  // 1. PERBAIKAN: Gunakan useEffect untuk memanggil data saat halaman dibuka
  useEffect(() => {
    fetchMentors();
  }, []); // Array kosong berarti dijalankan sekali saat mount
  ...
  // 2. PERBAIKAN: Tambahkan Optional Chaining (?.) agar tidak error jika data null
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredMentors.length > 0 ? (
          filteredMentors.map(m => (
            <div key={m.id} className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm hover:shadow-md transition-all group">
      ...
      {/* MODAL TAMBAH MENTOR (Kode Modal Tetap Sama) */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-[32px] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in duration-200 text-left">
            ...
            <form onSubmit={handleAddMentor} className="p-8 space-y-5">
              ...
              {/* Field Divisi & Max Kuota tetap sama */}
              <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold hover:bg-blue-700 shadow-lg flex items-center justify-center gap-2 mt-4">
                <CheckCircle size={20}/> Simpan Mentor
              </button>
            </form>
          </div>
        </div>
